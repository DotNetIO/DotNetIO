...
namespace DotNetIO.FileSystems.Local
{
	public class LocalFileSystemNotifierEntry : FileSytemNotifierEntry
	{
		...
		private void HandleFileEvent(object sender, FileSystemEventArgs e)
		{
			...
			IEnumerable<Action<File>> actions;
			lock (_actions)
			{
		...
		private class DisposableCookie : IDisposable
		{
			...
			private readonly Action<File>[] _notifications;
